...
def init_module(application): #initializes the passwords module
...
def password_model_factory(bind_key): #creates a password class/db model
    class DynamicPassword(db.Model, UserMixin):
        ...
        def __init__(self, user_id, password_hash, salt): #constructor
            self.id = user_id
            self.password_hash = password_hash
            self.salt = salt

        def __repr__(self) -> str: #string representation of password
            return f"""Password{'\n'}{self.id}{'\n'}{self.password_hash}{'\n'}{self.salt}{'\n'}"""


        def check_password_hash(self, password : str) -> bool: #salts and hashes a password from a user request and compares it with the hash in the db
            return generate_password_hash(password, self.salt) == self.password_hash
        
        def edit_password(self, password : str, db_session : Session = None) -> bool: #changes a user's password hash present in the passwords db
        ...
        def delete_password(self, db_session : Session = None) -> bool: #deletes a user password from passwords db
            if self == global_objects.admin_password:
                return False
            db_was_empty = db_session == None
            if db_was_empty:
...
def get_password_type(is_jobtaker : bool): #returns the password type for jobtakers or jobmakers
    if is_jobtaker:
        return JobTakerPassword
    else:
        return JobMakerPassword

def get_password_session(is_jobtaker : bool) -> Session: #returns a db session for jobtakers or jobmakers
    if is_jobtaker:
        return sessionmaker_jobtakerpassword()
    else:
        return sessionmaker_jobmakerpassword()

def get_password_by_id(user_id : int, is_jobtaker : bool) -> Tuple[Session, Query[Any]] | Any | None: #gets the password for a user id
...
def generate_salt(char_ammount : int = 16) -> str: #creates a random salt for a user
    chars = string.ascii_letters + string.digits
    return ''.join(choice(chars) for _ in range(char_ammount))

def generate_password_hash(password : str, salt : str) -> str: #takes in a password and salt and returns their hash
    hash = sha3_256()
    hash.update((password + salt).encode('utf-8'))
    return hash.hexdigest()

def add_user_password_to_db(user_id : int, password : str, is_jobtaker : bool, force_add : bool = False) -> bool: #add a password to the passwords db
    db_session = get_password_session(is_jobtaker)
    db_session.begin()
...
def delete_passwords_by_ids(user_ids : List[int], is_jobtaker : bool) -> bool: #deletes a password by a given user id
    db_session = get_password_session(is_jobtaker)
    user_type = get_password_type(is_jobtaker)
    db_session.begin()
