...
#handles job creation, deletion, editing

app : Flask

sessionmaker_job : sessionmaker

def init_module(application): #initializes the jobs module
...
class Job(db.Model):
    ...
    def __repr__(self) -> str: #string representation of a job
    ...
    def edit_job(self, request : Request, db_session : Session | None = None, commit : bool = True) -> bool: #validates and changes user accessible job data from a user request if job isn't locked
    ...
    def deny_jobtaker_job_state(self : Session | None = None, commit : bool = True): #for a jobmaker rejecting a jobtaker
    ...
    def progress_job_state(self, job_state : JobState, rating : int | None = None, jobtaker_id : int | None = None, db_session : Session | None = None, commit : bool = True) -> bool: #"progresses" job forward (reffer to chart)
        db_was_empty = db_session == None
        if db_was_empty:
            db_session : Session = sessionmaker_job()
            db_session.begin()
    ...
    def delete_job(self, db_session : Session | None = None, commit : bool = True) -> bool: #deletes job if it isn't locked
        if self.job_state.value >= JobState.ACCEPTED_MAKER.value:#if job_state is alr accepted, block job editing
            return False
        db_was_empty = db_session == None
        if db_was_empty:
...
def get_all_jobtaker_jobs(user) -> Tuple[Session, Query[Job]] | None: #returns a session and query with all of a jobtaker's jobs
...
def get_all_jobmaker_jobs(user) -> Tuple[Session, Query[Job]] | None: #returns a session and query with all of a jobmaker's jobs
...
def get_job_by_id(job_id : int) -> Tuple[Session, Query[Job]] | None: #gets job from db by id
    db_session : Session = sessionmaker_job()
    db_session.begin()
...
def create_job(user, request : Request) -> bool: #validates and creates a job from a user request
    if not validate_job_request(request):
        return False
...
def update_jobs_ratings(user) -> bool: #gets all jobs belonging to a user and updates their ratings to the user's rating
...
def validate_job_request(request : Request) -> bool: #checks if request.form contains parsable information
    def handle_error(e):
        print(e)
        return False
    try:
...
def get_filtered_jobs(user, request : Request, row_count : int = 20) -> Query[Job] | None: #filters for jobs
    db_session : Session = sessionmaker_job()
    db_session.begin()
    try:
    ...
    match job_filter:
        case JobFilter.DEFAULT | _:
            ...
            #todo: implement filtering/sorting by geographical proximity
