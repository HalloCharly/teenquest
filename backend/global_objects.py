...
admin_password_hash = "" #gets loaded from .env

#constants for accessing/creating dbs
...
#constants for identifying sessions
...
email_confirmation_salt : str = "" #gets loaded from .env
...
def init_module(application):#sets up flask Principals (role based acces control)
    global app
    app = application
...
def make_admin_object(admin_type):#creates and initializes the admin object
    global admin
    admin = admin_type("admin", "", "", '', Country('BE'), "", datetime.min, Gender.MALE, "x/y")
    admin.id = 0
...
def make_admin_password_object(admin_password_type): #creates and initializes the admin password object
